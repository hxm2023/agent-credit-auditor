...
def test_guard_cli_allow(tmp_path):
    ...
    env.write_text(
        json.dumps(
            {"schema_version": "grpo-guard-envelope-1.0", "required_extensions": [], "content_sha256": "a" * 64}
        ),
        encoding="utf-8",
    )
...
def test_guard_cli_reject_unknown_major(tmp_path, capsys):
    ...
    env.write_text(
        json.dumps(
            {"schema_version": "grpo-guard-envelope-2.0", "required_extensions": [], "content_sha256": "a" * 64}
        ),
        encoding="utf-8",
    )
