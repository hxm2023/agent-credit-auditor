...
def compute_identity_hashes() -> tuple[str, str]:
    ...
    tok_sha = hashlib.sha256(
        (Path(MODEL_PATH) / "tokenizer_config.json").read_bytes() + (Path(MODEL_PATH) / "tokenizer.json").read_bytes()
    ).hexdigest()
...
def base_manifest(policy_version: int, tok_sha: str, tpl_sha: str, run_name: str) -> dict:
    ...
    return {
        ...
        "adapter_kind": "lora",
        "adapter_sha256": None,
        "code_commit_sha": run_name,
        "config_sha256": hashlib.sha256(run_name.encode()).hexdigest(),
    }
...
def build_envelope(
    ...
    from grpo_guard.schema.envelope import TrainingContract, TrajectoryEnvelope
    ...
    auth_ref = (
        EventRef(uri="", event_id=scoring.event_id, event_sha256=scoring.event_sha256)
        if scoring
        else (EventRef(uri="", event_id=gen.event_id, event_sha256=gen.event_sha256))
    )
    ...
    return TrajectoryEnvelope(
        ...
        parent_identity_decision=EventRef(uri="", event_id=id_decision.event_id, event_sha256=id_decision.event_sha256)
        if id_decision
        else None,
        training_contract=TrainingContract(
            protocol="strict_on_policy",
            trainer_parent_policy_version=parent_ver,
...
def stage3_loss(model, handles, pos_weights, group_size, clip_epsilon=0.2):
    ...
    if total_weight <= 0:
        total_weight = 1.0
    return _t.tensor(loss_sum / total_weight, device=next(model.parameters()).device), {
...
def main() -> int:
    ...
    from grpo_guard.adapters.guarded_update import materialize
    ...
    canary_v0 = control.canary_passed(
        0,
        ckpt_cur["checkpoint_manifest_sha256"],
        epoch,
        sync_v0[0].sync_id,
        {"max_token_drift": 0},
    ...
    try:
        ...
        def train_sampler_rollout(policy_version: int) -> list[dict]:
            ...
            model.eval()  # noqa: F821 (closure over `model` defined in main)
            rows = []
            with _t.no_grad():
                for i in range(0, len(prompts), 8):
                    ...
                    for g in range(args.gens):
                        ...
                        logits = model(out).logits.float()  # noqa: F821
                        logps = _t.log_softmax(logits, dim=-1)
                        for j, p in enumerate(chunk):
                            plen = (
                                int(ids[j].ne(tokenizer.pad_token_id).sum().item())
                                if tokenizer.pad_token_id is not None
                                else int(ids.shape[1])
                            )
                            ...
                            rows.append(
                                {
                                    ...
                                    "text": text,
                                    "seq": cid,
                                    "pos": decision_positions(text, tokenizer, cid),
                                    "prompt_len": plen,
                                    "prompt_ids": ids[j, :plen].tolist(),
                                    "old_logprobs": lp,
                                }
                            )
            model.train()  # noqa: F821
            return rows
        ...
        def emit_and_validate(rows: list[dict], policy_version: int, source: str, epoch_no: int) -> list[dict]:
            ...
            for row in rows:
                ...
                gen = runtime.emit_generation(
                    row["prompt_ids"],
                    row["seq"],
                    row.get("old_logprobs") if source == "generation_service" else None,
                ...
                if source == "exact_behavior_scorer":
                    ...
                    lp_ref = store_.put(
                        lp_arr.tobytes(), "application/octet-stream", gen.event_id, dtype="float32", shape=[len(lp_arr)]
                    )
                    scoring = ScoringEvent(
                        event_id=f"scoring-{gen.event_id}",
                        event_type="behavior_scoring_finished",
                        run_id=run_id,
                        component_id="stage3-trainer",
                ...
                ctx = ValidationContext(
                    ...
                    store=store_,
                    events=all_events(),
                    policy_manifest=manifest_model(ckpt_cur),
                    split_manifest=split_model(split_manifest),
                    protocol=protocol,
                )
        ...
        for e in range(args.epochs):
            ...
            rows = emit_and_validate(
                train_sampler_rollout(policy_version), policy_version, "exact_behavior_scorer", epoch
            )
            ...
            if args.estimator == "local":
                ...
                for r_idx, row in enumerate(rows):
                    ...
                    pos_weights.append(pw)
                per_row = np.array(
                    [np.mean(list(pw["positions"].values())) if pw["positions"] else 0.0 for pw in pos_weights]
                )
            ...
            for row, a in zip(rows, per_row):
                ...
                rew = RewardEvent(
                    event_id=f"reward-{gen.event_id}",
                    event_type="reward_finished",
                    run_id=run_id,
                    component_id=args.task,
                    lifecycle_seq=next_lifecycle(),
                    created_at_utc=now_utc(),
                    input_events=[EventRef(uri="", event_id=gen.event_id, event_sha256=gen.event_sha256)],
                    reward_version="stage3-v1",
                    evaluator_protocol_sha256=sampling_sha,
                    source_generation_event=EventRef(uri="", event_id=gen.event_id, event_sha256=gen.event_sha256),
                    components={"utility": row["u"]},
                ...
                h = materialize(
                    store=store_,
                    run_id=run_id,
                    update_id=f"update-{policy_version + 1}",
                    preupdate_envelope=pre.ref(),
                    validation_decision=EventRef(
                        uri="", event_id=decision.event_id, event_sha256=decision.event_sha256
                    ),
                    sequence_ref=gen.sequence_token_ids,
                    loss_mask_ref=gen.loss_mask,
                    logprob_event_ref=EventRef(uri="", event_id=scoring.event_id, event_sha256=scoring.event_sha256)
                    if scoring
                    else pre.training_contract.authoritative_behavior_logprob_event,
            ...
            upd_input_refs = [
                EventRef(uri="", event_id=h.input_event.event_id, event_sha256=h.input_event.event_sha256)
                for h, _ in handles
            ]
            control.update_committed(
                update_id=f"update-{policy_version + 1}",
                transaction_id=f"txn-{e + 1}",
                lease_epoch=epoch,
                parent_policy_version=policy_version,
                output_policy_version=policy_version + 1,
            ...
            metrics["epoch_metrics"].append(
                {
                    ...
                    "mean_u": float(np.mean(utils_e)),
                    "success_rate": float(np.mean([u >= 0.99 for u in utils_e])),
                    "mean_calls": float(np.mean(n_calls)),
                    "invalid_rate": float(np.mean([c == 0 for c in n_calls])),
                    "grad_l2": grad_l2,
                    "loss": loss_res.metrics["loss"],
        ...
        with torch.no_grad():
            for p in eval_prompts:
                ...
                out = eval_model.generate(
                    ids, max_new_tokens=MAX_COMPLETION, do_sample=False, pad_token_id=tokenizer.eos_token_id
                )
